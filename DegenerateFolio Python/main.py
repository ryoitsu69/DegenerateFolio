...
binanceHas = ["MANA","BAT","GNO","SHIB","SAND","DAI","TUSD","MATIC","USDT","SUI",'APT',"CYBER","NEAR","OP","ARB","ETH","ID","BNB","AVAX","WOO","FTM","GLMR","STG","USDC","MATIC","DOT","MANTA","STRK","PYTH","JUP","SEI","TIA","ALGO","FIL", "SOL", "ATOM","ICP","LINK"]
...
def getAlgoTokens(address,update=True,waitToLoad=0):
...
def getSei(address,update=True,waitToLoad=0):
...
def getAtom(address,update=True,waitToLoad=0):
    ...
    try:
        ...
        time.sleep(waitToLoad)
        name = "ATOM"
        price = float(driver.find_element(By.XPATH,"/html/body/div[1]/div[2]/div/div/div[2]/div[1]/div[2]/div[1]/div[1]/div/div[1]/div[2]").text.split("(")[1][1:-1])
        amount = float(driver.find_element(By.XPATH,"/html/body/div[1]/div[2]/div").text.split(" ")[0])
        appendToken(Token(name,price=price,free=amount),update)

def getTia(address,update=True,waitToLoad=0):
...
def getSolTokens(address,update=True,waitToLoad=0):
...
def getStark(address,update=True,waitToLoad=0):
...
def getManta(address,update=True,waitToLoad=0):
...
def getAptos(address,update=True,waitToLoad=0):
...
def getSui(address,update=True,announcement = False,waitToLoad=0):
    print('im in getSui')
...
def getNearTokens(address,update=True,waitToLoad=0):
...
def getDot(address,update=True,waitToLoad=0):
...
def getEvmTokens(address,update=True,waitToLoad=0):
...
def getIcp(address,update=True,waitToLoad=0):
...
def getAllBalances(update):
    ...
    for network in networks:
        ...
        for wallet in network.wallets:
            try:
                tries+=1
                globals()[network.parseFunction](address=wallet,update=update,waitToLoad=retryIncrement*tries)
